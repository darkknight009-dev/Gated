...
interface AuditRow {
  /** ISO date already formatted for the current locale (server-computed). */
...
export function PrivacySettings({ emailAccount, preferences, audits }: Props) {
  ...
  return <div className="page-scroll"><div className="page-wrap"><header className="page-heading"><p className="eyebrow">Settings / Privacy center</p><h1>You set the boundaries.</h1><p>Control connection permissions, retention, analytics, and deletion from one place.</p></header>
    ...
    <section className="settings-section"><div><h2>Audit history</h2><p>Security-relevant account activity. Email bodies and OAuth tokens are never logged.</p></div><div className="settings-panel">{audits.length ? audits.map(({ created, eventTypeId, id }) => (
      <div className="settings-row" key={id}>
        <div><strong>{eventTypeId}</strong><p>{created}</p></div><Check size={14} className="muted" />
      </div>
