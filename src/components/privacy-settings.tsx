...
interface Props {
  ...
  audits: Array<{ id: string; event_type: string; created_at: string }>;
}
...
export function PrivacySettings({ emailAccount, preferences, audits }: Props) {
  ...
  return <div className="page-scroll"><div className="page-wrap"><header className="page-heading"><p className="eyebrow">Settings / Privacy center</p><h1>You set the boundaries.</h1><p>Control connection permissions, retention, analytics, and deletion from one place.</p></header>
    ...
    <section className="settings-section"><div><h2>Audit history</h2><p>Security-relevant account activity. Email bodies and OAuth tokens are never logged.</p></div><div className="settings-panel">{audits.length ? audits.map((event) => <div className="settings-row" key={event.id}><div><strong>{event.event_type.replaceAll(".", " ")}</strong><p>{new Date(event.created_at).toLocaleString()}</p></div><Check size={14} className="muted" /></div>) : <div className="settings-row"><p>No security events recorded yet.</p></div>}</div></section>
